...
A per-athlete \emph{mass factor} normalizes for body weight, since cycling power is absolute (watts) while VO$_2$Max is per-kilogram:
\begin{equation}
m_f \approx -0.034 \times \text{weight}_{\text{kg}} + 4.85
...
This fallback path is never exercised in our dataset (all cycling activities have \texttt{maxMet}), but is included for completeness.
...
\begin{enumerate}
    ...
    The running algorithm is insensitive to body weight (the ACSM equation does not use it), but the cycling fallback depends on it through the mass factor.
