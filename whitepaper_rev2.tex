...
The mass factor is empirically calibrated per athlete; it cannot be derived from a simple formula because it absorbs coupling between the linear coefficients $k$, $d$ and the athlete's physiological efficiency:
\begin{equation}
m_f = \text{per-athlete calibrated constant}
\label{eq:mass-factor}
\end{equation}
Calibrated values: Athlete~A ($68$\,kg) $\to$ $m_f = 0.210$; Athlete~B ($77$\,kg) $\to$ $m_f = 0.198$.
...
An athlete (68\,kg, HR$_{\text{rest}} = 42$, HR$_{\text{max}} = 176$, $m_f = 0.210$) completes a ride with $\overline{P} = 180$\,W and $\overline{\text{HR}} = 140$\,bpm:
\begin{align*}
...
The mass factor values and linear coefficients ($k$, $d$) were jointly optimized from a limited set of activities where \texttt{maxMet} was available as ground truth.
Applying this model to new athletes requires re-calibrating $m_f$; the two data points above are insufficient to derive a general weight-to-$m_f$ relationship.
...
Mass factor (Athlete~A) & $m_f$ & 0.210 \\
Mass factor (Athlete~B) & $m_f$ & 0.198 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    The mass factor values ($m_f = 0.210$, $0.198$) were jointly optimized with the linear coefficients and cannot be generalized to new athletes without re-calibration.
    ...
    The running algorithm is insensitive to body weight (the ACSM equation does not use it), but the cycling fallback depends on it indirectly through the mass factor calibration.
